...
export const EditClientPanelModal: React.FC<EditClientPanelModalProps> = ({
  ...
  useEffect(() => {
    if (complex) {
      ...
      setLogoUrl(complex.settings?.logoUrl || "");
        ...
        <form onSubmit={handleSaveAll} className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {activeTab === "GENERAL" && (
            <div className="space-y-4 animate-fadeIn">
              {/* Logo del Complejo */}
              <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30 flex flex-col sm:flex-row items-center gap-4">
                <div className="shrink-0">
                  {logoUrl ? (
                    <img
                      src={logoUrl}
                      alt="Logo"
                      className="w-16 h-16 rounded-xl object-cover ring-2 ring-indigo-500/20 shadow-xs bg-white dark:bg-slate-800"
                      onError={() => setLogoUrl("")}
                    />
                  ) : (
                    <div
                      style={{ backgroundColor: primaryColor }}
                      className="w-16 h-16 rounded-xl text-white flex items-center justify-center font-black text-xl shadow-xs"
                    >
                      {name ? name.substring(0, 2).toUpperCase() : "CD"}
                    </div>
                  )}
                </div>
                <div className="flex-1 w-full space-y-2">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
                    Logo del Complejo (URL directa o quitar)
                  </label>
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-xs focus:ring-2 focus:ring-indigo-500 focus:outline-hidden"
                    />
                    {logoUrl && (
                      <button
                        type="button"
                        onClick={() => setLogoUrl("")}
                        className="px-3 py-2 rounded-xl text-xs font-semibold text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40 border border-rose-200 dark:border-rose-900/50 cursor-pointer"
                      >
                        Quitar
                      </button>
                    )}
                  ...
                  <p className="text-[10px] text-slate-500">
                    Visible en el encabezado del panel de administración y en el portal público de clientes.
                  </p>
                </div>
              </div>
